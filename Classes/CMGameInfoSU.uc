class CMGameInfoSU extends ROGameInfoSupremacy;
		...
		foreach WorldInfo.AllPawns(class'Pawn', P)
		{
			if ( ROPawn(P) != none )
			{
				if ( P.PlayerReplicationInfo != none && P.PlayerReplicationInfo.Team != none )
				{
                    ...
                    //!
                    if (Objectives[i].ObjVolume == none && Objectives[i].bActive)
                    {
                        if (CMAObjective(Objectives[i]).CheckForPlayers(P.Location))
						{
                        	`log("I am in the OBJ! "$P.Location);
					...
					else if ( Objectives[i].bActive && Objectives[i].ObjVolume.ContainsPoint(P.Location) )
					{
						NumCappers[P.PlayerReplicationInfo.Team.TeamIndex]++;
